...
class CreateEntry extends Component<CreateEntryProp, CreateEntryState> {
  ...
  onChangeDownStream = (selected: any) => {
    var sid = selected;
    this.setState({
      downstream: sid,
    });
  ...
  onChangeAdminFlag = (selected: any) => {
    var sid = selected;
    this.setState({
      adminFlag: sid,
    });
  ...
  onSubmit(e: { preventDefault: () => void; }) {
    let selectorStrings: any[] = [], federatedWithList = [], dnsNamesWithList = [];
    e.preventDefault();
    ...
    if (this.state.federatesWith.length !== 0) {
      federatedWithList = this.state.federatesWith.split(',').map((x:any) => x.trim())
    }
    if (this.state.dnsNames.length !== 0) {
      dnsNamesWithList = this.state.dnsNames.split(',').map((x:any) => x.trim())
    }
