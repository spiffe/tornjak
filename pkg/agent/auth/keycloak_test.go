...
func TestNewKeycloakVerifier(t *testing.T) {
	// INIT failures
	_, err := NewKeycloakVerifier(true, "", "", "realm_access.roles")
	if err == nil {
		t.Fatal("ERROR: successfully initialized keyfunc for empty issuer url")
	}
	_, err = NewKeycloakVerifier(true, "invalid url", "", "realm_access.roles")
	if err == nil {
		t.Fatal("ERROR: successfully initialized keyfunc for invalid url")
	}

	_, err = NewKeycloakVerifier(false, "", "", "realm_access.roles")
	if err == nil {
	...
	_, err = NewKeycloakVerifier(false, sample_json, "", "realm_access.roles")
	if err != nil {
		t.Fatalf("ERROR: could not create keyfunc from json: %v", err)
	}

	if issuerURL != "" {
		_, err = NewKeycloakVerifier(true, issuerURL, "", "realm_access.roles")
		if err != nil {
